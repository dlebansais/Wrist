...
{
    public partial class change_password_failed__3Page : Page
    {
        public change_password_failed__3Page()
        {
            InitializeComponent();
            DataContext = this;
        }
        ...
        private void GoTo_Language_Switch__change_password_failed__3Page(object sender, RoutedEventArgs e)
        {
            string Content = (sender as Button).Content as string;
            Language.On_Switch("change password failed #3", "other language", Content);
            (App.Current as App).GoTo("change password failed #3");
        }
