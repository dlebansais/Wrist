...
{
    public partial class change_password_failed__2Page : Page
    {
        public change_password_failed__2Page()
        {
            InitializeComponent();
            DataContext = this;
        }
        ...
        private void GoTo_Language_Switch__change_password_failed__2Page(object sender, RoutedEventArgs e)
        {
            string Content = (sender as Button).Content as string;
            Language.On_Switch("change password failed #2", "other language", Content);
            (App.Current as App).GoTo("change password failed #2");
        }
