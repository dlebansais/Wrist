...
namespace Parser
{
    public class GeneratorUnitTesting : IGeneratorUnitTesting
    {
        ...
        private void Generate(string appNamespace, StreamWriter cSharpWriter)
        {
            ...
            cSharpWriter.WriteLine("        public Action<UnitTesting, DispatcherTimer, TestingOperation, Page, FrameworkElement> Handler;");
            cSharpWriter.WriteLine("    }");
            cSharpWriter.WriteLine();
            ...
            foreach (IGeneratorTestingOperation Operation in Operations)
            {
                ...
                if (Operation is IGeneratorClickOperation AsClick)
                {
                    ...
                    cSharpWriter.WriteLine("                Handler = (UnitTesting unitTesting, DispatcherTimer testTimer, TestingOperation operation, Page currentPage, FrameworkElement ctrl) =>");
                    cSharpWriter.WriteLine("                {");
                    cSharpWriter.WriteLine($"                    {PageName} page;");
                    cSharpWriter.WriteLine("                    Button ctrlButton;");
                    cSharpWriter.WriteLine($"                    if ((page = AssertPage<{PageName}>(unitTesting, testTimer, operation, currentPage)) != null)");
                    cSharpWriter.WriteLine("                        if ((ctrlButton = AssertControl<Button>(unitTesting, testTimer, operation, ctrl)) != null)");
                ...
                else if (Operation is IGeneratorToggleOperation AsToggle)
                {
                    ...
                    cSharpWriter.WriteLine($"                    if ((page = AssertPage<{PageName}>(unitTesting, testTimer, operation, currentPage)) != null)");
                    cSharpWriter.WriteLine("                        if ((ctrlButton = AssertControl<ToggleButton>(unitTesting, testTimer, operation, ctrl)) != null)");
                ...
                else if (Operation is IGeneratorFillOperation AsFill)
                {
                    ...
                    cSharpWriter.WriteLine($"                    if ((page = AssertPage<{PageName}>(unitTesting, testTimer, operation, currentPage)) != null)");
                    cSharpWriter.WriteLine($"                        if ((ctrlEdit = AssertControl<{ControlType}>(unitTesting, testTimer, operation, ctrl)) != null)");
                    cSharpWriter.WriteLine("                        {");
            ...
            cSharpWriter.WriteLine("                Operation.Handler(this, TestTimer, Operation, CurrentPage, Ctrl);");
            ...
            cSharpWriter.WriteLine("        }");
            cSharpWriter.WriteLine();
            cSharpWriter.WriteLine("        private static T AssertPage<T>(UnitTesting unitTesting, DispatcherTimer testTimer, TestingOperation operation, Page page) where T : Page");
            cSharpWriter.WriteLine("        {");
            cSharpWriter.WriteLine("            if (!(page is T))");
            cSharpWriter.WriteLine("            {");
            cSharpWriter.WriteLine("                unitTesting.Abort(testTimer, operation, \"Invalid page.\");");
            cSharpWriter.WriteLine("                return null;");
            cSharpWriter.WriteLine("            }");
            cSharpWriter.WriteLine();
            ...
            cSharpWriter.WriteLine("        }");
            cSharpWriter.WriteLine();
            cSharpWriter.WriteLine("        private static T AssertControl<T>(UnitTesting unitTesting, DispatcherTimer testTimer, TestingOperation operation, FrameworkElement ctrl) where T : FrameworkElement");
            cSharpWriter.WriteLine("        {");
            ...
            cSharpWriter.WriteLine("                {");
            cSharpWriter.WriteLine("                    unitTesting.Abort(testTimer, operation, \"Invalid control.\");");
            cSharpWriter.WriteLine("                    return null;");
            cSharpWriter.WriteLine("                }");
            cSharpWriter.WriteLine();
